...
def preflight(
    allowed_methods=None, allowed_headers=None,
    allow_credentials=False, max_age=None, origins=None
):
...
class CORS(object):
    ...
    def preflight(self, allowed_methods, allowed_headers, allow_credentials,
                  max_age, origins):
        if self._is_valid_preflight_request(
            allowed_headers, allowed_methods, origins
        ):
            self._add_origin_and_credentials_headers(allow_credentials)
    ...
    def _is_valid_preflight_request(
        self, allowed_headers, allowed_methods, origins
    ):
        return (
...
def _safe_caching_headers():
    ...
    """
    uah: str = cherrypy.serving.request.headers.get('User-Agent', '')
    if uah.find('MSIE') == -1:
